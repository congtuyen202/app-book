  ...
  <div class="bg-background text-foreground">
    <Head>
      ...
      <template v-if="post?.tags">
        <Meta v-for="tag in post.tags" :key="`tag-${tag.id}`" property="article:tag" :content="tag.name" />
      </template>
    </Head>
    ...
    <article v-else class="py-8">
      <header class="container mx-auto px-4 max-w-3xl text-center mb-10">
        ...
        <div class="text-sm text-muted-foreground">
          <span>By {{ post.author?.name || 'Unknown Author' }}</span> &bull;
          <span>Published on {{ formatDate(post.publishedDate) }}</span>
          <span v-if="post.category">
            &bull; In
            <NuxtLink :to="`/blog/category/${post.category.slug}`" class="hover:underline">
              <Badge variant="outline">{{ post.category.name }}</Badge>
            </NuxtLink>
          </span>
        </div>
        <div v-if="post.tags && post.tags.length > 0" class="mt-4">
          <NuxtLink v-for="tag in post.tags" :key="tag.id" :to="`/blog/tag/${tag.slug}`" class="mr-1.5 mb-1.5 inline-block">
...
const { data: post, pending: pendingPost, error: fetchError } = await useAsyncData<BlogPost | undefined>(
  ...
  async () => {
    try {
      const response: any = await $axios.post('http://localhost:8000/graphql', {
      ...
      if (response.data.errors) {
        ...
        if (response.data.data?.bookaiBlogBySlug === null || errorMessage.toLowerCase().includes('not found')) {
      ...
      // Check if error is already an H3Error or has a statusCode (like from $fetch)
      if (e.statusCode) {
        statusCode = e.statusCode;
        statusMessage = e.data?.message || e.statusMessage || e.message || statusMessage;
      } else if (e.message) {
        statusMessage = e.message;
      }

      // Avoid overriding a 404 if it was already set (e.g. from the try block's showError)
      const existingError = fetchError.value; // Nuxt's useAsyncData error ref
      if (!(existingError && existingError.statusCode === 404)) {
